...
void GeometryAnalyzer::compute(const Mesh& mesh, int quad_order) {
  const int num_nodes = mesh.nodeCoords.size();
  const int num_elements = mesh.elementNodes.size();
...
void GeometryAnalyzer::initialize_containers(const Mesh& mesh, int mint) {
  int num_nodes = mesh.nodeCoords.size();
  int num_elements = mesh.elementNodes.size();

  nodeNormals.assign(num_nodes, Vec3(0.0, 0.0, 0.0));
  nodeContribCount.assign(num_nodes, 0);
...
#pragma omp for schedule(static)
    for (int k = 0; k < num_elements; ++k) {
      ...
      for (int i = 0; i < 6; ++i)
        nodes[i] = mesh.nodeCoords[mesh.elementNodes[k][i]];
...
#pragma omp for
    for (int elem = 0; elem < num_elements; ++elem) {
      ...
      for (int local = 0; local < 6; ++local) {
        int global = mesh.elementNodes[elem][local];
...
void GeometryAnalyzer::compute_moment_matrix(const Mesh& mesh, const QuadratureData& quad, int mint) {
  int num_elements = mesh.elementNodes.size();  // Changed from mesh.n to mesh.elementNodes
...
#pragma omp for
    for (int k = 0; k < num_elements; ++k) {
      std::vector<Vec3> nodes(6);
      for (int i = 0; i < 6; ++i)
        nodes[i] = mesh.nodeCoords[mesh.elementNodes[k][i]];  // Changed from mesh.p to mesh.nodeCoords
...
#pragma omp critical
  ...
  Mmat = Mmat_accum.lu().inverse();  // Final assignment with matrix inversion
}
...
void GeometryAnalyzer::print_summary(const Mesh& mesh, int num_print) const {
  ...
  for (int i = 0; i < std::min(num_print, (int)mesh.nodeCoords.size()); ++i) {
    const auto& n = nodeNormals[i];
    const auto& p = mesh.nodeCoords[i];  // updated from mesh.p
...
#pragma omp for
    for (int k = 0; k < num_elements; ++k) {
      const auto& tri = mesh.elementNodes[k];  // Changed from mesh.n to mesh.elementNodes
      const Vec3& A = mesh.nodeCoords[tri[0]];  // Changed from mesh.p to mesh.nodeCoords
      const Vec3& B = mesh.nodeCoords[tri[1]];  // Changed from mesh.p to mesh.nodeCoords
      const Vec3& C = mesh.nodeCoords[tri[2]];  // Changed from mesh.p to mesh.nodeCoords
