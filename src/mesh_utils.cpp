...
// Check for repeated node indices within each element
bool check_mesh_integrity(const Mesh& mesh) {
  bool all_good = true;

#pragma omp parallel for
  for (int k = 0; k < mesh.elementNodes.size(); ++k) {  // Changed 'n' to 'elementNodes'
    std::set<int> node_indices;
    bool duplicate = false;

    for (int i = 0; i < 6; ++i) {
      int idx = mesh.elementNodes[k][i];  // Changed 'n' to 'elementNodes'
      if (node_indices.count(idx)) {
#pragma omp critical
        {
          std::cerr << "Duplicate node index " << idx
                    << " in element " << k << ": [";
          for (int j = 0; j < 6; ++j) std::cerr << mesh.elementNodes[k][j] << " ";  // Changed 'n' to 'elementNodes'
          std::cerr << "]\n";
        }
        duplicate = true;
        break;
...
// Check triangle orientation using signed area (nodes 0,1,2)
bool check_triangle_orientation_strict(const Mesh& mesh, double threshold) {
  bool all_ccw = true;
...
#pragma omp for nowait
    for (int k = 0; k < mesh.elementNodes.size(); ++k) {  // Changed 'n' to 'elementNodes'
      const auto& tri = mesh.elementNodes[k];  // Changed 'n' to 'elementNodes'

      Vec3 A = mesh.nodeCoords[tri[0]];  // Changed 'p' to 'nodeCoords'
      Vec3 B = mesh.nodeCoords[tri[1]];  // Changed 'p' to 'nodeCoords'
      Vec3 C = mesh.nodeCoords[tri[2]];  // Changed 'p' to 'nodeCoords'

      Vec3 normal = (B - A).cross(C - A).normalized();  // Local triangle normal
      Vec3 centroid = (A + B + C) / 3.0;
      Vec3 radial = centroid.normalized();              // Expected outward direction
